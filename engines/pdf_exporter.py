...
# ── Cover page ────────────────────────────────────────────────────────────────
def _cover_section(itin, constraints, st, prepared_by=None):
    elems = []
    ...
    if prepared_by:
        elems.append(Spacer(1, 2 * mm))
        elems.append(Paragraph(f"Prepared by: <b>{prepared_by}</b>", st["body_small"]))

    # AI disclaimer
    elems.append(Spacer(1, 4 * mm))
    elems.append(HRFlowable(width="100%", thickness=0.5, color=PDF_BORDER,
                             spaceAfter=4, spaceBefore=4))
    elems.append(Paragraph(
        "<i>This document was generated by RouteIQ\u2019s AI engine. "
        "While RouteIQ strives for accuracy, AI-generated itineraries may contain "
        "errors or omissions. All route plans should be reviewed and verified by a "
        "qualified logistics professional before dispatch.</i>",
        st["about_small"]
    ))
    return elems
...
# ── Warnings & notes page ─────────────────────────────────────────────────────
_LLM_WARN_KEYWORDS = ("llm", "offline", "rule-based", "rule based", "fallback")


def _notes_section(itin, st):
    all_warnings = itin.get("warnings") or []
    # Strip system/LLM-status warnings — only keep operational warnings
    warnings = [
        w for w in all_warnings
        if w and not any(kw in w.lower() for kw in _LLM_WARN_KEYWORDS)
    ]
...
# ── Public API ────────────────────────────────────────────────────────────────
def generate_itinerary_pdf(itin: dict, constraints: dict = None,
    ...
    story += _cover_section(itin, constraints or {}, st, prepared_by=prepared_by)
